  ...
  >
    ...
    <div
      ...
      <div class="uplaod-action" :style="{height: miniWidth / 4 + 'px'}">
        <i v-if="item.url" class="iconfont icon-tupianyulan" title="预览" @click="handlePreviewFile(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
        <i class="iconfont icon-sync1" v-show="!disabled" title="替换" @click="handleEdit(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
        ...
        <i class="iconfont icon-delete" v-show="!disabled" title="删除" @click="handleRemove(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async uplaodAction (res, file, key) {
      ...
      try {
        const { content } = await uploadFile(formData)
        const info = content.info

        if (this.fileList.findIndex(item => item.id === info.id) >= 0) {
          this.fileList.splice(changeIndex, 1)
        ...
        this.$set(this.fileList, changeIndex, {
          ...this.fileList[changeIndex],
          id: info.id,
          percent: 100,
          status: 'success'
        })
      ...
      handler (val) {
        ...
        ids.forEach(async (id, index) => {
          try {
            ...
            const url = file ? URL.createObjectURL(new Blob([ file ])) : ''
            this.$set(fileList, index, {
              ...fileList[index],
              url,
              status: 'success'
            })
