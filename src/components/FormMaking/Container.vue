          ...
          <div class="components-list">
            ...
            <div class="components-list">
              ...
              <template v-if="JSON.stringify(mainForm) != '{}'">
                ...
                <draggable
                  tag="ul"
                  v-model="mainForm.data"
                  v-bind="{
                    group: { name: 'grid', pull: true, put: false },
                    sort: false,
                    ghostClass: 'ghost'
                  }"
                  @end="handleMoveEnd"
                ...
                >
                  <li
                    class="form-edit-widget-label"
                    :class="{ 'no-put': item.type == 'divider' }"
                    v-for="(item, index) in mainForm.data"
                    :key="index"
                  >
                    <el-tooltip
                      class="item"
                ...
                <div v-if="!mainForm.data.length">
                  <span style="font-size:12px;padding: 8px 20px;color:#F56C6C"
                    >暂无字段</span
                  >
                </div>
              </template>
              ...
              <template v-if="subForm.length">
                <div v-for="sub in subForm" :key="sub.formId">
                  ...
                  <draggable
                    ...
                    :list="sub.data"
                    v-bind="{
                      group: { name: sub.formId, pull: true, put: false },
                      sort: false,
                      ghostClass: 'ghost'
                    }"
                    @end="handleMoveEnd"
        ...
        <el-container class="center-container" direction="vertical">
          <el-header class="btn-bar" style="height: 45px;">
            ...
            <el-button
              ...
              type="text"
              size="medium"
              icon="element-icons el-custom-initialize"
              @click="handleInitial"
              >&nbsp初始化</el-button
            >
            <el-button
            ...
            <el-button
              ...
              type="text"
              size="medium"
              icon="element-icons el-custom-save"
              @click="handleSave"
              >&nbsp;&nbsp;保存</el-button
            >
            <el-button
          ...
          <el-container>
            ...
            <el-main class="config-content">
              <widget-config
                ...
                :allSubForms="allSubForms"
                :usedSubForms="usedSubForms"
                @removeCol="removeCol"
              ></widget-config>
              <form-config
...
export default {
  ...
  computed: {
    ...mapGetters(['historyStates', 'stateIndex']),
    allSubForms() {
  ...
  methods: {
    ...
    transModelData() {
      ...
      model.main = {
        ...
        data: _.cloneDeep(
          this.initialTransform(this.modelData.id, this.modelData.datas)
        )
      }
      let textKey =
      ...
      for (let i = 0; i < loop; i++) {
        ...
        modelTemp.main.data[i * 2].model =
          modelTemp.main.data[i * 2].options.model
        ...
        if (i == loop - 1 && modelTemp.main.data.length % 2 == 1) {
          //最后一次循环且数量为奇数
        } else {
          modelTemp.main.data[i * 2 + 1].key =
            modelTemp.main.data[i * 2 + 1].options.key
          modelTemp.main.data[i * 2 + 1].model =
            modelTemp.main.data[i * 2 + 1].options.model
    ...
    removeWidget({ index, removeData }) {
      ...
      if ('text' != type) {
        //不是文本
        if (formId == this.mainForm.formId) {
          //是主表单的
          let removeModelData = this.dataBak.main.data.filter(d => {
            return d.options.key == key
          })
