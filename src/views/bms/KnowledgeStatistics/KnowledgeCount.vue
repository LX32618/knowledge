  ...
  <el-container>
    <el-main>
      ...
      <el-row v-else style="margin-top:-35px">
        <cs-table ref="tb" :settings="tableSettings" :table-data="tableData">
          <template v-slot:horizontalSlot>
            <div class="toolBar">
              ...
              >
                ...
                <el-button type="primary" icon="el-icon-s-grid" slot="reference"
                  >设置表头</el-button
                >
              </el-popover>
              <el-button
                type="primary"
                icon="element-icons el-custom-export"
  ...
  methods: {
    chooseHeader() {
      ...
      this.$set(
        this.tableSettings,
        'fields',
        _.concat(this.tableFieldsBak, headerChecked)
      )
    ...
    tableExport() {
      const tHeader = this.tableSettings.fields
        .filter(c => {
      ...
      const filterVal = this.tableSettings.fields
        .filter(c => {
          return c.visible != false
        })
        .map(c => {
    ...
    async drawChart() {
      ...
      if ('dept' == this.dimension) {
        let option = {
          sql:
            "select so.DEPARTNAME as \"name\",count(1) as \"y\" from knowledge_base k,T_S_DEPART so ,fd_org_userrel re where k.creator = re.userid and  re.deptid = so.id and k.classification not in (SELECT ID FROM DOC_CATEGORY START WITH ID in ('','') CONNECT BY PRIOR ID = PID) and k.isdel = 0 and so.ORGTYPEID='402803967e6aec39017e6bce896900d3' and k.audit_status = 1 group by so.DEPARTNAME,so.DEPTCODE order by so.DEPTCODE",
          sqlwhere: ''
        }
        ...
        let option = {
          sql:
            'select d.categoryname as "name",count(1) as "y" from knowledge_base k ,doc_category d where k.baseid = d.id and k.isdel = 0 and k.audit_status = 1 group by d.categoryname',
          sqlwhere: ''
        }
        let resp = await fetchIndKnowledgeCount(option)
