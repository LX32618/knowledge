<template>
  <el-form
    ...
    :rules="rules"
    ref="basicForm"
    :label-width="settings.lableWidth"
  >
    ...
    <el-form-item label="类型" v-if="data.pid != ''">
      <el-select
        placeholder="请选择目录类型"
        v-model="data.type"
        :disabled="settings.formType == 'basic'"
      >
        <el-option
          label="知识库"
          :value="0"
          v-if="settings.formType == 'basic' || data.isRoot"
        ></el-option>
              ...
              <div class="operationNav">
                ...
                <el-button-group class="search">
                  <el-input
                    ...
                    @keyup.enter.native="searchAssociatedForm"
                  ></el-input>
                  <el-button type="primary" @click="searchAssociatedForm"
                    >搜索</el-button
                  >
                  <el-button type="primary" @click="certainAssociatedForm"
                    >确定</el-button
                  >
                </el-button-group>
              </div>
      ...
      >
        <div>
          <cs-lazytree
            ...
            @checkChange="labelCheckChange"
            style="overflow: auto;height: 250px;"
          ></cs-lazytree>
          <el-button
            type="primary"
            size="mini"
            style="float: right;margin:0px 3px 3px 0px"
            @click="certainLabel"
            >确定</el-button
          >
        </div>
    ...
    <p :style="{ textIndent: '2em' }">
      <i class="element-icons el-custom-files"></i
      >(分类)：表示该目录类型下还可以添加子分类和目录，不能挂知识文档
    </p>
    <p :style="{ textIndent: '2em' }">
      <i class="element-icons el-custom-file"></i
      >(目录)：表示该目录类型下不能再添加子目录和分类了，能够挂知识文档
    </p>
    <el-button
      type="primary"
    ...
    formData: {
      handler(newVal, oldVal) {
        ...
        if (this.settings.formType == 'append') {
          if (newVal.isRoot) this.data.type = 0
          //根节点下新增是知识库
          else this.data.type = 1 //新增默認是分類
        }
  ...
  methods: {
    ...
    treeDataFormat({ node, data }) {
      ...
      let formatData = temp.map((item, index, arr) => {
        let pid = item.pid
        if (this.treeSettings.root_id == pid) {
          //如果是根节点
    ...
    certainAssociatedForm() {
      ...
      this.$set(
        this.data,
        'formName',
        this.selectedAssociatedFormTable.formName
      )
